export const NAME = "dim";
export const VERSION = "v1.1";
export const DEFAULT_DATAFILES_PATH = "./data_files";
export const DEFAULT_DIM_FILE_PATH = "./dim.json";
export const DEFAULT_DIM_LOCK_FILE_PATH = "./dim-lock.json";
export const DIM_FILE_VERSION = "1.1";
export const DIM_LOCK_FILE_VERSION = "1.1";
export const DEFAULT_SEARCH_ENDPOINT = "https://search.ckan.jp/backend/api/package_search";
export const OPENAPI_COMPLETIONS_ENDPOINT = "https://api.openai.com/v1/completions";
export const ENCODINGS = [
  "EUC-JP",
  "ISO-2022-JP",
  "Shift_JIS",
  "UTF-8",
  "UTF-16",
  "UTF-16BE",
  "UTF-16LE",
  "UNICODE",
];
